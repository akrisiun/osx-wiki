...
# [String]$DriveLetter = "D:" # (Get-Diskimage -ImagePath $ServerISO | Get-Volume).DriveLetter
...
[String]$NanoServer_wim = "d:\VM\nano-Work\NanoServer.wim"
[String]$NanoServer_Folder = "d:\VM\nano-Work\"
...
# else {
 ...
 #   . .\Convert-WindowsImage
 #   Convert-WindowsImage `
 #       -Sourcepath "$($NanoServer_Folder)\NanoServer.wim" `
 #       -VHD $TempVHD `
 ##       –VHDFormat $VHDFormat `
  #      -Edition $Edition `
  #      -VHDPartitionStyle $VHDPartitionStyle        
...
foreach ($Package in $PackageList) {
    If ($Package.Name -in $Packages) {
	    ...
	    & "$DismFolder\Dism.exe" '/Add-Package' "/PackagePath:$($NanoServer_Folder)\packages\$($Package.Filename)" "/Image:$MountFolder"
	    & "$DismFolder\Dism.exe" '/Add-Package' "/PackagePath:$($NanoServer_Folder)\packages\en-us\$($Package.Filename)" "/Image:$MountFolder"
    }
}
